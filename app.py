...
from sqlalchemy import Column, DateTime, PrimaryKeyConstraint, Time
...
class Show(db.Model):
    ...
    venue_id = db.Column("venue_id", db.Integer, db.ForeignKey(
        "Venue.id"))
    artist_id = db.Column("artist_id", db.Integer, db.ForeignKey(
        "Artist.id"))
...
class Venue(db.Model):
    ...
    artists = db.relationship('Artist', secondary='Show', lazy=True)
...
class Artist(db.Model):
    ...
    seeking_description = db.Column(db.String(120))
    venues = db.relationship('Venue', secondary='Show', lazy=True)
...
def venues():
    ...
    for venue in Venue.query.all():
        ...
        upcoming_shows = Show.query.filter_by(
            venue_id=venue.id).where(Show.start_time > datetime.now()).count()
        found = False
        for locations in data:
            if locations["city"] == location[0] and locations["state"] == location[1]:
                locations["venues"].insert(i, {
                    "id": venue.id,
                    "name": venue.name,
...
def edit_venue_submission(venue_id):
    # TODO: take values from the form submitted, and update existing
    # venue record with ID <venue_id> using the new attributesform = ArtistForm(request.form)
    form = VenueForm(request.form)
    venue = Venue.query.get(venue_id)
    try:
